        ...
        if ($feeds['name'] == "{}rss") {
            ...
            foreach ($loopers as $loop) {
                ...
                if ($loop['name'] == "{}item") {
                    ...
                    foreach ($values as $value) {
                        ...
                        if ($value['name'] == '{}link') {
                            ...
                            if (isset($value['value']) && strlen($value['value']) > 0) {
                                ...
                                //Load any url:
                                $info = \Cohensive\Embed\Facades\Embed::make($url)->parseUrl();
                    ...
                    $optimized_feeds['channel']['item'][] = ['render_type' => $renderType, 'featured_image' => $featuredImage, 'video_embed_code' => $videoEmbedCode, 'title' => $title, 'link' => $link, 'description' => $description, 'pubDate' => $pubDate];
                }
            }
        }
